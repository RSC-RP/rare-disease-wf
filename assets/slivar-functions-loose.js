// This is a modified version of slivar-functions.js from brentp/slivar.
// It does no genotype quality checking at all.  If you use it, please check
// the genotype quality on the output variants to make sure you really believe it!
...
function x_recessive(kid, mom, dad) { 
  return (mom.het && kid.AB > 0.75 && dad.hom_ref && kid.alts >= 1 
              && kid.sex == 'male')
}
...
function segregating_denovo_x(s) {
  if(s.sex == "female") {
    ...
    if(s.affected) { return s.het && parents_x_dn_or_homref(s) }
    return s.hom_ref
  }
  if(s.sex == "male") {
