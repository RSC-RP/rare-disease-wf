...
If you haven't already, set up a directory to store apptainer images that you
have downloaded.
...
apptainer pull docker://brentp/somalier
```

Now you can give it a quick test to see that you have access to the software.

```{bash eval = FALSE}
apptainer exec ~/apptainer/somalier_latest.sif somalier extract --help
```
...
Apptainer can only see your home directory, which is why I have to bind
`/gpfs/shared_data`.
...
apptainer exec --bind /gpfs/shared_data:/mnt \
~/apptainer/somalier_latest.sif \
...
apptainer exec --bind $REPODIR:/mnt \
~/apptainer/somalier_latest.sif somalier relate \
