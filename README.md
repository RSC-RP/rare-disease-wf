...
to perform variant calling, starting from BAMs or FASTQs from trios, duos and/or
...
This workflow comes with tiny example BAMs and a
...
Now you can look in the "results" folder and see some files that were generated.

### Running your own data

Go into the `params` folder and make a copy of the YAML file that matches the
version of the genome that you are using. Fill in and modify parameters as
needed to point to your inputs and association. You can look at the datasets in
`testdata/` to see how to format your files.
The example dataset is a VCF, but it is fine to use a gzipped VCF or a BCF.

Say you called your parameters file `myparams.yml`. You would run variant calling
like this:
...
And then variant filtering and trio analysis like this:
...
Nextflow work directory (`temp_raredisease` in your user directory within your
association by default; delete this temp directory once you are done).

For the `calltrios` workflow, you will get a subfolder called `gvcfs` containing
one gVCF per sample, generated by DeepTrio/DeepVariant. gVCFs will be named according to
...
Variant calling was performed on BAM files using DeepTrio v1.8.0
